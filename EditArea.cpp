...
//编写新的传递系统表和系统列信息的函数
int CEditArea::iReadDictstruct(char tabname[][20],int *tabnum,char colname[][20][20],int colnum[],AttrType coltype[][20],int collength[][20],int coloffset[][20],int iscolindex[][20])
{
	CHustBaseDoc* pDoc = (CHustBaseDoc*)GetDocument();
	...
	GetCurrentDirectory(BUFFER, lpszCurDir); 
	...
	//	condition.LattrLength=strlen(tabname[i])+1;
		...
		while(GetNextRec(&FileScan2,&rec2)==SUCCESS)//这个函数需要实现
		{
...
void CEditArea::showSelResult(int row_num, int col_num) {
	...
	CPen pen1(PS_SOLID, 3, RGB(100, 100, 100));	//外框显示的颜色
	CPen * pOldPen = dc.SelectObject(&pen1);
	dc.Polyline(pframe, 4);//画了三条线
	dc.MoveTo(point4);	//画第四条线
	dc.LineTo(point1);
	dc.SelectObject(pOldPen);
	
	POINT pBegin, pEnd;
	
	for (i = 1; i<col_num; i++)	{//画框图的几条
		pBegin.y = 1;
		pBegin.x = 180*i+1;
		pEnd.y = row_num*25+1;
		pEnd.x = 180*i+1;
